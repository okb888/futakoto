...
## 📊 進捗サマリー（2026-05-09 最終更新）

> v3 レビュー（[2026-05-09-review-v3.md](./2026-05-09-review-v3.md)）で追加された課題は別ファイルを参照。
...
| P1-1 | 解釈キャッシュ invalidate | ✅ 完了（2026-05-09 v3: CF をパートナー1名に絞る形で実装） |
| P1-2 | Firebase Auth エラー日本語化 | ✅ 完了（2026-05-08） |
| P1-3 | メール認証 `sendEmailVerification` | ✅ 完了（2026-05-09 自動実行） |
| P1-4 | ログアウト確認ダイアログ | ✅ 完了（2026-05-08） |
| P1-5 | `COLORS`トークン全画面適用 | ✅ 完了（2026-05-09 v3: 全ブランド色リテラルを完全排除） |
| P1-6 | 重複コンポーネント抽出 | ✅ 完了（2026-05-09 v3: `lib/format.ts` / `lib/profile.ts` に集約） |
| P1-7 | デッドコード削除 | ✅ 完了（2026-05-09 自動実行） |
| P1-8 | AGENTS.md 更新 | ✅ 完了（2026-05-08） |
| P1-9 | 通知タップ時のディープリンク | ✅ 完了（2026-05-09 自動実行） |
| P1-10 | アクセシビリティラベル最低限付与 | ✅ 完了（2026-05-09 自動実行） |
| P1-11 | 利用規約 | ✅ 完了（2026-05-09 自動実行） |
| P2-1 | `consultations` collection 整理 | ✅ 完了（2026-05-09） |
| P2-2 | TimePickerSheet スクロール追従 | ✅ 完了（2026-05-09） |
| P2-3 | 壁打ちセッション「続きから再開」 | ✅ 完了（2026-05-09） |
| P2-4 | オンボーディング/初回CTA強化 | 💤 保留 |
| P2-5 | 17+レーティング判断 | ✅ 完了（判断のみ、12+で申告）|

### v3 タスク（2026-05-09 追加 → 当日完了）

| タスク | タイトル | 状態 |
|--------|---------|------|
| V3-CRIT-1 | `index.tsx` ArrowRight import 漏れ（ホームクラッシュ） | ✅ 完了（2026-05-09） |
| V3-CRIT-2 | `invalidateInterpretationCache` の全ユーザー走査 | ✅ 完了（2026-05-09） |
| V3-CRIT-3 | `aiInterpret` entryOwner 検証なし | ✅ 完了（2026-05-09） |
| V3-CRIT-4 | `aiConsult` conversationHistory がクライアント任せ | ✅ 完了（2026-05-09 sessionId 駆動に変更） |
| V3-FN-1 | `aiConsult` UX 分離（reflection のみ + `aiDraftOptions` / `aiDraft` 新設） | ✅ 完了（2026-05-09） |
| V3-FN-2 | 壁打ち「新しく始める」に未保存確認ダイアログ | ✅ 完了（2026-05-09） |
| V3-FN-3 | 投稿削除時の `favorites` / `interpretationCache` 孤児 | ✅ 完了（2026-05-09 `cleanupOnEntryDelete` 新設） |
| V3-FN-4 | 共有範囲「直前の選択を覚える」 | ✅ 完了（2026-05-09） |
| V3-CQ-1 | format / partnerName util 抽出 | ✅ 完了（2026-05-09） |
| V3-CQ-4 | プロンプトインジェクション軽減 | ✅ 完了（2026-05-09） |
| V3-UI-1 | リテラル色を COLORS トークンに全廃 | ✅ 完了（2026-05-09） |
| V3-UX-4 | 解釈「もう一度読み解く」ボタン | ✅ 完了（2026-05-09） |

### デプロイ状況

全 Cloud Functions を `asia-northeast1` にデプロイ済み（2026-05-09）。

### 残課題（リリース前必須）

| タスク | タイトル |
|--------|---------|
| P0-5 | TestFlight 実機検証（未着手。これが唯一の残 P0） |

### 残課題（中・低優先）

| タスク | タイトル |
|--------|---------|
| V3-CQ-2 | ホームの混合ソート見直し（実害低） |
| V3-CQ-3 | `Entry.createdAt` 型整理（ripple 大） |
| V3-UI-2/3 | UI 一貫性の細部 |
...
**判断結論（2026-05-09）**:
- ふたことはペアリングした2名のクローズドな共有のみ。第三者アクセス不可 → **12+** で申告可能
- App Store Connect の「コンテンツの説明」セクション: 「ユーザーが作成したコンテンツ」= No（第三者に公開されないため）
- 通報機能は現時点では実装しない（クローズド運用のため不要）。将来Open化する際に追加
...
- **2026-05-09 深夜（自動実行）**: P1-3（sendEmailVerification）・P1-6（SourceConsultationLinkコンポーネント化）・P1-7（App.tsx/デッドコード削除）・P1-9（通知ディープリンク）・P1-1（解釈キャッシュinvalidate CF追加）・P1-10（アクセシビリティラベル）・P1-11（terms.html作成・設定画面にリンク追加）・P2-1（consultations→consultationSessions移行）・P2-2（TimePickerSheet scrollRef追従）・P2-3（壁打ち「続きから再開」）・P2-5（12+レーティング判断記録）を完了。git push 済み。
...
## 🧪 仮案：aiConsult UX設計変更（2026-05-08 検討中）

> **ステータス：仮案。eval結果を踏まえて正式タスク化を判断する。**

### 課題
- `aiConsult` が毎ターン `messageDraft` を返している
- 相談が終わっていない段階でドラフトが出るのはUX的に不自然
- 毎ターンのdraft生成がAPI費用の無駄になっている

### 設計変更方針

**壁打ち（相談）とメッセージ生成を完全分離する。**
...
「メッセージにする」ボタン（ユーザーが明示的に押したとき）
  → aiDraftOptions: 会話履歴を読んで3択+自由記述を動的生成（新関数）
...
### 3択の生成方式
- **A案（固定）**: 「気持ちを伝えたい」「お願いしたい」「感謝したい」+ 自由記述
- **B案（AI動的生成）**: 会話履歴を読んでその人の状況に合った3択を提案 ← 採用方針

B案を採用する理由：相談内容が多様になると固定3択では合わないケースが出る（例：自己反省混じり、事実共有だけしたいなど）。「メッセージ化」ボタン押下時のみ発火するためAPI増加の影響は小さい。

### 変更対象
- `functions/src/index.ts`: `aiConsult` から `messageDraft` を削除。`aiDraftOptions`・`aiDraft` を新規追加
- `lib/ai.ts`: 対応するクライアント関数を追加
- `app/(app)/consult.tsx`: 「メッセージにする」ボタン追加・4択モーダル実装

### コスト比較（概算）
| 現状 | 変更後 |
|------|--------|
| 相談n回 × (reflection + messageDraft) | 相談n回 × reflection のみ + ボタン押下時2回 |
| 毎ターン課金 | ユーザーが望んだときだけ課金 |
